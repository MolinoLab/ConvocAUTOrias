...
| `10-procesar-recordatorios.json` | Recordatorios `/recordar` pendientes para hoy (fecha o siempre) | Diario 8:00 (TZ: Europe/Madrid) |
...
- El workflow **08** envía el resumen semanal a los mismos chats (`TELEGRAM_CHAT_ID` + `TELEGRAM_NOTIFY_CHAT_IDS`). Usa `CALDAV_AGENDA_CALENDAR_NAMES` (o `CALDAV_CALENDAR_NAME` con varios nombres separados por coma) para calendarios de equipo/compartidos, más todos los calendarios personales de `CALDAV_PERSONAL_CALENDAR_BY_TELEGRAM`; los mensajes incluyen hora de generación local.
- El workflow **10** envía recordatorios creados con `/recordar` al chat donde se crearon (no usa `TELEGRAM_NOTIFY_CHAT_IDS`). Fecha concreta: un aviso ese día a las 8:00. `siempre`: aviso diario a las 8:00 hasta `/rmrecordar`.
